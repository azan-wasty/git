...
FROM Creature C
WHERE C.CreatureID = (
	SELECT R.CreatureID FROM Rank R WHERE Level = (
		SELECT MAX(Level) FROM Rank
	)
)
...
WHERE I.IngredientID = (
	...
	ORDER BY COUNT(CreatureID) DESC
)
...
WHERE C.CreatureID IN (
	(SELECT TOP 1 Creature1ID FROM Fight
	GROUP BY Creature1ID
	ORDER BY COUNT(Creature1ID) DESC)
	UNION
	(SELECT TOP 1 Creature2ID FROM Fight
	GROUP BY Creature2ID
	ORDER BY COUNT(Creature2ID) DESC)
)
...
FROM Creature C
WHERE C.CreatureID IN (
	(SELECT Creature1ID FROM Fight WHERE Creature1ID = WinnerID)
	UNION 
	(SELECT Creature2ID FROM Fight WHERE Creature2ID = WinnerID)
)
...
FROM Creature C
WHERE C.CreatureID = (
	SELECT TOP 1 Creature1ID
	FROM ( 
		(SELECT Creature1ID FROM FamilyRelation WHERE RelationType = 'Sibling'
		UNION 
		SELECT Creature2ID FROM FamilyRelation WHERE RelationType = 'Sibling')
	) AS sibling
	ORDER BY Creature1ID DESC
)
...
WHERE C.CreatureID IN (
	...
	GROUP BY Creature1ID
	HAVING COUNT(Creature1ID) >= 2)
	UNION
	(SELECT Creature2ID FROM Fight
	GROUP BY Creature2ID
	HAVING COUNT(Creature2ID) >=2)
)
...
SELECT Name FROM Creature WHERE CreatureID = (
	SELECT TOP 1 Creature1ID FROM 
	(
		(SELECT Creature1ID, Creature2ID FROM Fight) 
		UNION  
